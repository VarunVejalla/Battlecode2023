...
public class Headquarters extends Robot {
    ...
    public boolean shouldISaveUp() throws GameActionException {
        ...
        int numIslandsToWin = 3 * numIslands / 4 - numFriendlyIslands;
        int roundsUntilNextAnchor = roundNum < 1800 ? 60 : 25;

        if(numEnemyLaunchersNearby > 0){
            return false;
        }

        if(roundNum < 20 || roundsSinceLastAnchor < roundsUntilNextAnchor){
            return false;
        }
        ...
        if (0.1 > (double) distToIsland/rc.getMapHeight()/rc.getMapWidth())
            heuristic += 3;
...
//        Util.log("Seen carriers: " + carrierToRoundMap.size());
       ...
       // lastAd = rc.getResourceAmount(ResourceType.ADAMANTIUM);
       // lastMana = rc.getResourceAmount(ResourceType.MANA);
        int roundInd = rc.getRoundNum()%10;
